...
export async function getInterventionsByDispositiveIdHandler(
  ...
  try {
    ...
    const result = await monitoringService.getInterventionsByDispositiveId(dispositiveId);
    return reply.code(200).send({
      success: true,
      data: result
    });
  } catch (error) {
    console.error('Error fetching interventions by dispositive ID:', error);
    return reply.code(500).send({
      success: false,
      message: 'Failed to fetch interventions by dispositive ID'
    });
  }
}
